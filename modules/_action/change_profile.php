...
if($type=='update_info') {
	$db->table = "core_user";
	$db->condition = "`user_id` = " . $db->clearText($user_id) . " and `password` = '" . $db->clearText(md5($_SESSION["admin_user"].$_POST['passwordold'])) . "'";
	$db->order = "";
	$db->select();
	if($db->RowCount>0) {
		...
		if($OK) {
			...
			if($handleUploadImg) {
				...
				if($imgUp->processed) {
					...
					echo "<script>alert('Lỗi tải hình: " . $imgUp->error . "')</script>";
				}
...
} else if($type=='update_pass') {

	$db->table = "core_user";
	$db->condition = "`user_id` = " . $db->clearText($user_id) . " and `password` = '" . $db->clearText(md5($_SESSION["admin_user"].$_POST['password2old'])) . "'";
	$db->order = "";
	$db->select();
	if($db->RowCount>0) {
		$db->table = "core_user";
		$data = array(
			'password'=>$db->clearText(md5($_SESSION["admin_user"].$_POST['password']))
		);
		$db->condition = "user_id = ".$user_id;
		$db->update($data);
		echo "<script>alert('Thực hiện đổi mật khẩu thành công.');</script>";
		echo showChangePassword();
		echo '<head><meta http-equiv="Refresh" content="3, ?active=change-password-success"></head>';
		die();
	} else {
		echo "<script>alert('Nhập mật khẩu hiện tại không đúng, vui lòng kiểm tra lại.')</script>";
		echo showChangePassword();
