		...
		if ($db->RowCount > 0) {
			foreach ($rows as $row) {
				...
				foreach ($rws as $rw) {
					...
					$message = $notify_type[($rw["type"])+0] . ': ' . stripslashes($rw["msg"]);
				}
				if($row['status']==0) {
					$status = ' class="new"';
				...
				echo '<li><a' . $status . ' href="javascript:;" onclick="return open_notify($(this), ' . $row['notify'] . ');"><span class="photo">' . $agent[4] . '</span><span class="subject"><span class="from">' . $agent[0] . '</span><span class="time">' . convertTimeAgo($created_time) . '</span></span><span class="message">' . $message . '</span>' . '</a></li>';
			}
		}
		...
		if($db->RowCount>0) {
			...
			foreach($rows as $row) {
				...
				foreach($rws as $rw) {
					$agent = getInfoUser($rw["user_id"]);
					$created_time = $rw["created_time"];
					$message = $notify_type[$rw["type"]+0] . ': ' . ($rw["msg"]);
				}
				if($row['status']==0) {
					$status = ' class="new"';
					$btn_sm = '<i class="fa fa-eye" onclick="return marked_notify($(this), ' . $row['notify'] . ');" data-toggle="tooltip" data-placement="top" title="Chưa xem" rel="1"></i>';
			...
			foreach ($rows as $row) {
				...
				if ($db->RowCount == 0 && $total<3) {
					...
					foreach ($rws as $rw) {
						$agent = getInfoUser($rw["user_id"]);
						$calendar = $rw["calendar"];
						$message = '[' . $agent[0] . '] ' . $notify_type[$rw["type"]] . ': ' . getContentCalendar($calendar);
						$icon = $agent[11];
					}
					...
					array_push($result, array('title' => "CÔNG THÀNH - LỊCH HỌP", 'tag' => $row['notify'], 'body' => $message, 'icon' => $icon));
					$total++;
				}
			}
		}
