...
if (!defined('TTH_SYSTEM')) { die('Please stop!'); }
 $ol = (isset($_GET['ol']) && $_GET['ol'] !== '')? $_GET['ol'] : 'document';
		...
		$op = (isset($_GET['op']) && $_GET['op'] !== '')? $_GET['op'] : 'document_list_move';


		$fun = isset($_GET['fun']) ? $_GET['fun'] : 'canhan';
		
		$overdue = isset($_GET['overdue']) ? $_GET['overdue'] : '';
	
		
?>
<div class="row in-relative mak">

	<div class="col-md-7 locdanhsach">

		<div class="col-md-12">

			<form id="_from_search_exhibition" method="post" onsubmit="return ol_filter('_from_search_exhibition', '_search_exhibition', 'search_exhibiton');">
			<div class="row">

			<select name="status" class="tech form-control chaclangu rowtotal" onchange="valselect(this.value);" id="tech000" data-toggle="tooltip" data-placement="top" title="" data-original-title="Trạng thái đơn hàng" >

						<option value="0" selected data-image="<?= HOME_URL;?>/images/tich.png">Chọn trạng thái...</span></option>
						<?php 
                            $db->table = "status";
							$db->condition = "";
							$db->order = "`status_id` ASC";
							$db->limit = 6;
							$rows = $db->select();
							foreach ($rows as $stat):
						?>
						<option value="<?= $stat['status_id'] ?>" data-image="<?= HOME_URL;?>/images/<?=getStatusColor($stat['status_id']);?>.png"><?= $stat['description'] ?></option>
						<?php endforeach ?>
						
			</select>
			<span class="getusername">
			<select name="member" class="tech form-control chaclangu" id="tech00">

						<option value="0" selected data-image="<?= HOME_URL;?>/images/icon-cus.png">Chọn nhân viên...</option>
						<?php 
                            $db->table = "core_user";
							$db->condition = "";
			...
			<input type="text" value="" placeholder="Nhập khách hàng" name="namecustom" class="timkhach" autocomplete="off"  />


			</div>

			<div class="row">
			<div style="margin-top: 10px;" class="divtim">
			<button type="submit" name="list" class="btn btn-round color-1 material-design nuthemmuc nutin" style="margin-left: 0px; margin-right: 0px;"><i class="fa fa-search"></i>Tìm kiếm </button>
			</div>
			</div>
			</form>			
		</div>
	...
	<div class="col-md-5 col-xs-12">		
		...
		<ul class="nh_locbang" title="xử lý nội dung">	
			...
			<li class="dropdown">
				...
				<ul class="dropdown-menu extended logout">
					...
					<li><a href="/?ol=<?= $ol ?>&op=<?= $op ?>&fun=canhan&overdue=<?=$overdue ?>"><i class="fa fa-calendar-plus-o"></i>Tôi</a></li>
                                        
                    <li class="divider"></li>
					
                    <li><a href="/?ol=<?= $ol ?>&op=<?= $op ?>&fun=tochuc&overdue=<?=$overdue ?>"><i class="fa fa-calendar"></i>Tất cả</a></li>
		...
		<?php if(in_array(3,$roles)) { ?><span class="cusa"><button class="btn btn-round color-1 material-design nuthemmuc" onclick="Forward('<?=HOME_URL;?>/?ol=document&op=document_add')" style="float: right;margin-bottom:15px;"  name="login_admin" data-color="#004740" data-toggle="tooltip" data-placement="top" title="" data-original-title="Thêm đơn hàng mới"><i class="fa fa-plus" ></i> <span>Thêm đơn hàng</span></button></span>

		<?php } 
		if(in_array(3,$roles)) { ?><span class="cusa"><a class="btn btn-round color-1 material-design nuthemmuc overdue"  href="/?ol=document&op=document_list_move&fun=<?=$fun ?>&overdue=ok" name="login_admin" style="background-color: #ff4740; color: #fff;line-height:20px; float: right;margin-bottom: 10px;" data-toggle="tooltip" data-placement="top" title="" data-original-title="Đơn hàng trễ"><i class="fa fa-exclamation-triangle" ></i> <span>Đơn hàng trễ</span></a></span>
		<?php } ?>
		...
		<div class="col-lg-12 no-relative" >
			...
			<div class="panel panel-no-border">
				...
				<div class="table-responsive" id="_search_exhibition">
						...
						<table class="table table-no-border table-ol-3w table-hover danhsachdonhangcanhan" id="dataTablesList">
							...
							<tbody class="taplet" id="sortable">
								...
								<?php
						$extra_cond = "";
						if($overdue =='ok') $extra_cond = " AND date_end < " . time(); 
						$stt = 1;
						...
						if($db->RowCount > 0 ) {
							...
							$db->table="exhibition";
							$db->condition = " `status` > ". $status . " AND status < 12 AND `exhibition_id` = " . $row['id_exhibition']. $extra_cond;
							$db->order = "`created_time` DESC ";
							$db->limit = "";
							$row2s = $db->select();
							if($db->RowCount > 0 ) {
						...
						?>
										...
										<tr class="bangkhachhang"  id="bang<?=  $row2['exhibition_id']  ?>" >
											...
											<td>
												...
												<select name="tech" class="tech form-control sectcustum1 " <?php if(in_array("document;status", $corePrivilegeSlug['op'])) echo ''; else echo 'disabled="disabled"';?> id="tech<?=$stt++;?>" onchange="change_state(this,<?=$row2['exhibition_id'];?>,'<?=$row2['code']?>');"style="padding-left: 20px;">
													...
													<option value="13"  data-image="<?= HOME_URL;?>/images/<?=getStatusColor(13)?>.png">Tạm hoãn</option>
														...
														<table class="table table-no-border table-ol-3w table-hover danhsachdonhangcanhan" id="dataTablesList">
															...
															<thead>
																...
																<tr class="panel-heading thantran" >
																	...
																	<th  width="150px;"><label class="form-lb-tp">Code1</label></th>
