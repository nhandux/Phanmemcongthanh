...
if($typeFunc=='add'){
	if(empty($title)) $error = '<span class="show-error">Vui lòng nhập địa điểm.</span>';
		...
		$db->insert($data);
		loadPageSuccess("Đã thêm dữ liệu thành công.", TTH_PATH_LK . $link_ol[4] . TTH_PATH_OP_LK . $link_op[4][4]);
		$OK = true;
	}
}
else {
...
if(!$OK) jobs_addressGeneral(TTH_PATH_LK . $link_ol[4] . TTH_PATH_OP_LK . 'general_jobs_address_add', "add", 0,  $title, $slug, $is_active, $error);