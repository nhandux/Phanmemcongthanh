...
$ol_agency  = getAgencyToCore($corePrivilegeSlug['op'], '/statistic_exhibition;/');
...
<div class="row">
	<div class="col-md-12">
		<ul class="breadcrumbs-alt">
			...
			<li>
				<a class="current"><?php echo $menu_op[6][0];?></a>
			</li>
		</ul>
	</div>
</div>
...
<div class="row">
	<div class="col-lg-12">
		<div class="row">
			<form id="_from_statistic_exhibition" method="post" onsubmit="return ol_filter('_from_statistic_exhibition', '_statistic_exhibition', 'statistic_exhibiton');">
				<div class="input-statistic clearfix formxn">
					...
					<select name="status" class="tech form-control chaclangu rowtotal" id="tech" data-toggle="tooltip" data-placement="top" title="" data-original-title="Trạng thái đơn hàng" onchange="valselect(this.value);"">
					...
					<select name="startotal" class="tech form-control chaclangu "  onchange="rowtotal(this.value)"  id="tech1" style="float: left;margin-top:10px;" onchange="ol_filter1('_from_statistic_exhibition', '_statistic_exhibition', 'statistic_exhibiton');"">

						<option value="0" selected data-image="<?= HOME_URL;?>/images/lich.png">Từ tổng giá trị...</span></option>
						
						<?php 
						$ic = 0;
                            while($ic <= 100000000){
						?>
						<option value="<?= $ic ?>" ><?= number_format($ic,'0','','.') ?> VNĐ</option>
						<?php
						$ic = $ic + 1000000; } 
						?>
					...
					<span class="coltotal">
					    <select name="endtotal" class="tech1 form-control chaclangu"  disabled=""  id="tech2" style="float: left;margin-top:10px;">
					...
					<input type="text"  style="float: left;margin-top:10px !important;"  class="form-control formngayh ncs" name="name_customer" placeholder="Từ hiện tại..." autocomplete="off" data-toggle="tooltip" data-placement="top" title="" data-original-title="Thống kê theo khách hàng">
                   
                <div class="radiobot tiepradio" >
				     <div class="checkbox checkbox-danger" style="margin-right: 5px;margin-top: 7px;">
                        <input id="checkbox6" type="checkbox" checked="checked"  name="urgent" value="0" >
                        <label for="checkbox6">
                            Cá nhân
                        </label>
                    </div>
					...
					<div class="checkbox checkbox-success" style="margin-top: 7px;">
                        <input id="checkbox3" type="checkbox" checked="checked" name="pick_up" value="0" >
                        <label for="checkbox3">
                            Tổ chức
                        </label>
                    </div>
                ...
                <script>
					...
					<div class="pull-right custumstatis ">
						...
						<button type="button" name="export" class="btn btn-round color-1 material-design nuthemmuc saoluudulieu nutin" onclick="return ol_print_export('_from_statistic_exhibition', 'statistic_jobs_export');">Báo cáo word <i class="fa fa-file-word-o"></i></button>
						<button type="reset" name="list" class="btn btn-round color-1 material-design nuthemmuc nutin" style="background: #ff6600">Làm mới <i class="fa fa-magic"></i></button>
		...
		<div class="panel panel-no-border f-space10">
			<div class="table-responsive" id="_statistic_exhibition">
				...
				$(document).ready(function(e) {
					$("#payments").msDropdown({visibleRows:4});
					$(".tech2").msDropdown().data("dd");//{animStyle:'none'} /{animStyle:'slideDown'} {animStyle:'show'}		
					//no use
					try {
						var pages = $("#pages").msDropdown({on:{change:function(data, ui) {
																var val = data.value;
